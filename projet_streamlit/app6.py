...
def create_lagged_features(df, target_col="height", lags=5):
    df_lagged = df.copy()
    for i in range(1, lags + 1):
        df_lagged[f"lag{i}"] = df_lagged[target_col].shift(i)
    df_lagged.dropna(inplace=True)
    X = df_lagged[[f"lag{i}" for i in range(1, lags + 1)]]
    y = df_lagged[target_col]
    return X, y


def predire_et_afficher(df, model, nb_jours_a_predire=30, lags=3, target_col="water_height", frequence_jours=15, ic_constante=0.9102):
    """
    Prédit les niveaux d'eau avec un intervalle de confiance fixe (ex: ±0.9102 m).
    """
    data = df.copy()
    ...
    if len(data) < lags:
        st.warning(f"Il faut au moins {lags} valeurs pour prédire avec {lags} lags.")
        return None

    # Récupère les derniers lags dans le bon ordre chronologique
    niveaux = data[target_col].iloc[-lags:].tolist()
    predictions = []
    start_date = data.index[-1]

    for i in range(nb_jours_a_predire):
        # Génère les features lag dans le bon sens
        features = {f"lag{j+1}": niveaux[-lags + j] for j in range(lags)}
        X_pred = pd.DataFrame([features])
        y_pred = model.predict(X_pred)[0]
        niveaux.append(y_pred)

        next_date = start_date + timedelta(days=frequence_jours * (i + 1))
        predictions.append((next_date, y_pred))

    df_pred = pd.DataFrame(predictions, columns=['date', 'niveau_eau_pred'])
    df_pred['lower'] = df_pred['niveau_eau_pred'] - ic_constante
    df_pred['upper'] = df_pred['niveau_eau_pred'] + ic_constante

    # Plot dans Streamlit
    fig, ax = plt.subplots(figsize=(12, 6))

    date_cutoff = start_date - timedelta(days=90)
    df_recent = data[data.index >= date_cutoff]

    ax.plot(df_recent.index, df_recent[target_col], label="Valeurs réelles", color='blue')
    ax.plot(df_pred['date'], df_pred['niveau_eau_pred'], label="Prédictions", color='orange', linestyle='--')
    ax.fill_between(df_pred['date'], df_pred['lower'], df_pred['upper'], color='orange', alpha=0.2, label="IC ±95% (±0.91 m)")

    ax.set_title(f"Niveau d'eau - Derniers 3 mois + {nb_jours_a_predire * frequence_jours} jours de prédiction")
    ax.set_xlabel("Date")
...
if choix != "-- Aucune sélection --" and 'df_filtré' in locals() and not df_filtré.empty:
    ...
    st.subheader("Prédictions futures avec VotingRegressor")

    if st.button("Afficher les prédictions futures avec VotingRegressor"):
        df_prediction = df_model.copy().rename(columns={"height": "water_height"}).reset_index(drop=True)
        lags = 30  # ou slider Streamlit
        X, y = create_lagged_features(df_model, target_col="height", lags=lags)
        ...
        # crée un nouveau modèle Voting avec les bons lags
        ...
        df_pred = predire_et_afficher(
            df=df_prediction,
            model=voting_model,  # 👈 le modèle qu’on vient de recréer
            nb_jours_a_predire=30,
            lags=lags,
            target_col="water_height",
            frequence_jours=15
